    ...
    onComplete: function () {
        if (isDashBoardReport) {
            ...
            if (fs.existsSync('./testreport')) {
                shelljs.cd('./testreport');
                shelljs.exec('Syncfusion.UpdateTestResults.exe /Platform:"JavaScript - EJ2" /Control:"' + components +
                    '" /TestingTool:"Selenium" /StartTime:"' + timeCal.startTime + '" /EndTime:"' + new Date().toLocaleTimeString() +
